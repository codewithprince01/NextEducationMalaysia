      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs">
        <div>
          <h1 className="text-xl font-bold text-slate-800 tracking-tight flex items-center gap-2">
            <Globe className="w-5 h-5 text-emerald-800" /> International Student Data Countries
          </h1>
              ...
              {loading ? (
                ...
                paginated.map((item, idx) => (
                  <tr key={item.id} className="hover:bg-[#f6fcf8] transition-colors">
                    ...
                    <td className="py-3.5 px-4 font-bold text-slate-800">{item.country_name}</td>
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
                ...
                >
                  ...
                  <span>{editingId ? 'Save Changes' : 'Create Country'}</span>
                </button>
              </div>
            </form>
          </div>
