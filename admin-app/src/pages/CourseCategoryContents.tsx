...
export default function CourseCategoryContents() {
  const { id: routeCategoryId } = useParams<{ id: string }>();
  const navigate = useNavigate();

  const [categories, setCategories] = useState<CategoryOption[]>([]);
  const [selectedCategoryId, setSelectedCategoryId] = useState<number | null>(
    routeCategoryId ? Number(routeCategoryId) : null
  );
  ...
  // Fetch Contents for selected category
  const fetchContents = async (catId: number) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/v1/admin/course-category-contents?course_category_id=${catId}`);
      const json = await res.json();
      if (res.ok && json.data) {
  ...
  const handleCategorySelectChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    navigate(`/course-category-contents/${val}`);
    setEditingId(null);
    setFormData({
      tab: 'Overview',
      position: 1,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedCategoryId) {
      showToast('error', 'Please select a course category first');
      return;
    }
    ...
    try {
      const url = editingId
        ? `/api/v1/admin/course-category-contents/${editingId}`
  ...
  const handleDelete = async (id: number, tabTitle: string) => {
    ...
    try {
      ...
      if (res.ok && json.status) {
        ...
        if (selectedCategoryId) fetchContents(selectedCategoryId);
      } else {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200/80 shadow-xs">
        <div className="flex items-center gap-4">
          <Link
            ...
            title="Back to Course Categories"
          >
          ...
          <div>
            ...
            <h1 className="text-xl font-extrabold text-slate-900 tracking-tight mt-0.5">
              {selectedCategoryObj ? `Contents for "${selectedCategoryObj.name}"` : 'Manage Category Contents'}
            </h1>
            <p className="text-xs text-slate-500 font-medium mt-0.5">
              Create, edit and format rich tab descriptions for course category detail pages.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <label className="text-xs font-bold text-slate-600 shrink-0">Select Category:</label>
          <select
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input
                type="text"
                required
                placeholder="e.g. Overview, Admission, Fees, Syllabus"
                value={formData.tab}
          ...
          {/* Form Action Buttons (Matching Laravel style: Reset & Submit) */}
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="p-12 text-center text-slate-400">
            ...
            <p className="text-xs text-slate-400 mt-1">Use the rich editor form above to enter description and create content tabs.</p>
          </div>
        ) : (
