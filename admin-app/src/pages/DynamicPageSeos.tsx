...
export default function DynamicPageSeos() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.url.trim()) {
      showToast('error', 'Target URL is required');
      return;
    }
    ...
    <div className="space-y-3 max-w-[1600px] mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-xl border border-slate-200/80 shadow-sm">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Manage meta tags and custom HTML page content for dynamic target URLs.
          </p>
        </div>
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200/80 shadow-sm flex flex-col sm:flex-row gap-4 justify-between items-center">
        <div className="relative w-full sm:w-80">
          ...
          <input
            type="text"
            placeholder="Search URL or meta title..."
            value={searchQuery}
              ...
              {loading ? (
                ...
                paginated.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="py-3.5 px-4 font-semibold text-indigo-600 font-mono text-xs">{item.url}</td>
          ...
          <div className="bg-white rounded-xl shadow-2xl border border-slate-100 w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-5 border-b border-slate-100 bg-slate-50/50">
              <h3 className="text-base font-semibold text-slate-800">
                {editingId ? 'Edit Dynamic Page SEO' : 'Add Dynamic Page SEO'}
              </h3>
              <button
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4 overflow-y-auto flex-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    placeholder="e.g. /uploads/og-dynamic.jpg"
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 uppercase mb-1.5">
                  Custom Page Content (WYSIWYG)
                </label>
                <RichTextEditor
                  value={formData.page_content}
                  onChange={(content) => setFormData({ ...formData, page_content: content })}
                  placeholder="Enter custom HTML/WYSIWYG page content..."
                />
              </div>
