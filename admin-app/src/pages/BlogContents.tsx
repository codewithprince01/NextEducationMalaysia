...
export default function BlogContents() {
  const [items, setItems] = useState<BlogContentItem[]>([]);
  ...
  const handleOpenEdit = (item: BlogContentItem) => {
  ...
  const handleDelete = async (id: number) => {
    const isConfirmed = await confirmDelete('Are you sure you want to delete this content section?');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.title.trim()) {
      showToast('error', 'Heading title is required');
      return;
    }
    ...
    try {
      const url = editingId
        ? `/api/v1/admin/blog-contents/${editingId}`
        : '/api/v1/admin/blog-contents';
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 sm:p-5 rounded-2xl border border-slate-100 shadow-2xs">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 mt-1">
            Manage sub-headings, tabs, and detailed article body sections for blogs.
          </p>
        </div>
        <div className="flex items-center gap-2.5">
          ...
          >
            <Plus className="w-3.5 h-3.5 text-emerald-300" /> Add Content Section
          </button>
        </div>
      </div>

      {/* Controls */}
      <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-2xs flex flex-col sm:flex-row gap-3 justify-between items-center">
        <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
          <div className="relative w-full sm:w-80">
            <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
            <input
              type="text"
              placeholder="Search heading title..."
              value={searchQuery}
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="py-2 px-3 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:outline-hidden focus:ring-1 focus:ring-emerald-500 focus:border-emerald-600 text-slate-700 max-w-xs font-medium"
            >
          ...
          <table className="w-full text-left text-xs text-slate-600">
            <thead className="bg-[#effaf2] border-b-2 border-[#c8ebd2] text-[#14532d] font-extrabold uppercase tracking-wider text-[11px]">
              <tr>
                ...
                <th className="py-3.5 px-4">Heading Title</th>
              ...
              {loading ? (
                <tr>
                  <td colSpan={6} className="py-12 text-center text-slate-400">
                    ...
                    Loading blog content sections...
                  </td>
                </tr>
              ) : paginated.length === 0 ? (
                <tr>
                  <td colSpan={6} className="py-12 text-center text-slate-400 font-medium">
                    No blog content sections found.
                  </td>
                </tr>
              ) : (
                paginated.map((item, idx) => (
                  <tr key={item.id} className="hover:bg-slate-50/80 transition-colors">
                    <td className="py-3.5 px-4 text-center font-bold text-emerald-700">{(currentPage - 1) * itemsPerPage + idx + 1}</td>
                    <td className="py-3.5 px-4 font-bold text-emerald-900 max-w-xs truncate">
                    ...
                    <td className="py-3.5 px-4 text-center font-mono text-xs text-slate-600">{item.position || 0}</td>
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-100 w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-5 border-b border-slate-100 bg-slate-50/50">
              <h3 className="text-base font-bold text-slate-800">
                {editingId ? 'Edit Blog Content Section' : 'Add Blog Content Section'}
              </h3>
              <button
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4 overflow-y-auto flex-1">
              ...
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. Why Study in Malaysia?"
