...
export default function Authors() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (res.ok && json.status) {
        ...
        showToast('error', json.message || 'Failed to fetch authors');
      }
    } catch {
      showToast('error', 'Network error while fetching authors');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    const isConfirmed = await confirmDelete('Are you sure you want to delete this author?');
    if (!isConfirmed) return;

    try {
      const res = await fetch(`/api/v1/admin/authors/${id}`, { method: 'DELETE' });
      const json = await res.json();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name.trim()) {
      showToast('error', 'Author name is required');
      return;
    }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 sm:p-5 rounded-2xl border border-slate-100 shadow-2xs">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 mt-1">
            Manage article authors, editors, profiles, and designations.
          </p>
        </div>
        <div className="flex items-center gap-2.5">
          ...
          >
            <Plus className="w-3.5 h-3.5 text-emerald-300" /> Add Author Profile
          </button>
        </div>
      </div>
              ...
              {loading ? (
                ...
                paginated.map((item, idx) => (
                  <tr key={item.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="py-3.5 px-4 font-medium text-emerald-800">{item.email || '-'}</td>
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-100 w-full max-w-lg overflow-hidden">
            <div className="flex items-center justify-between p-5 border-b border-slate-100 bg-slate-50/50">
              <h3 className="text-base font-bold text-slate-800">
                {editingId ? 'Edit Author Profile' : 'Add Author Profile'}
              </h3>
              <button
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              ...
              <div>
                ...
                <input
                  type="email"
                  placeholder="sarah@educationmalaysia.in"
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="e.g. Senior Academic Counselor & Editor"
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3.5 py-2 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:outline-hidden focus:ring-1 focus:ring-emerald-500 focus:border-emerald-600 focus:bg-white transition-all resize-none"
                />
              </div>
