    ...
    <div className="space-y-5 max-w-[1600px] mx-auto pb-8">
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-7 border border-stone-200/90 shadow-xs relative overflow-hidden">
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-4 pt-3.5 border-t border-stone-100 text-xs">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-[#effaf2] border border-[#c8ebd2]/70 text-[#14532d]">
            <span className="text-emerald-700 text-[11px] font-medium uppercase tracking-wider">Active Staff:</span>
            <span className="font-extrabold text-[#14532d]">{loading ? '...' : items.filter((u) => Number(u.status) === 1).length}</span>
          </div>
          ...
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-stone-100/70 border border-stone-200 text-stone-800">
            <span className="text-stone-600 text-[11px] font-medium uppercase tracking-wider">Delegated:</span>
            <span className="font-extrabold text-stone-900">{loading ? '...' : items.filter((u) => (u.role || '').toLowerCase() !== 'admin' && Number(u.status) === 1).length}</span>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
          <div className="relative w-full sm:w-80">
            ...
            <input
              ...
              className="w-full bg-stone-50/70 border border-stone-200/90 rounded-xl pl-10 pr-8 py-2 text-xs font-medium text-stone-800 placeholder-stone-400 focus:outline-hidden focus:ring-1 focus:ring-emerald-500 focus:border-emerald-600 focus:bg-white transition-all"
            />
            {searchQuery && (
              <button
                type="button"
          ...
          <div className="relative w-full sm:w-48">
            <select
              ...
              className="w-full bg-stone-50/70 border border-stone-200/90 rounded-xl px-3 py-2 text-xs font-bold text-stone-700 focus:outline-hidden focus:ring-1 focus:ring-emerald-500 focus:border-emerald-600 focus:bg-white cursor-pointer"
            >
          ...
          {(searchQuery || roleFilter) && (
            <button
              ...
              className="px-3 py-2 rounded-xl text-xs font-bold text-emerald-800 hover:bg-[#effaf2] cursor-pointer transition-colors"
            >
              Reset Filters
            </button>
          )}
            ...
            <tbody className="divide-y divide-stone-100 font-medium text-stone-700">
              {loading ? (
                ...
                paginated.map((item, idx) => (
                  <tr key={item.id} className="hover:bg-[#effaf2]/40 transition-colors group">
                    <td className="py-4 px-4 text-center font-bold text-emerald-700 font-mono text-[11px]">
                      {(currentPage - 1) * itemsPerPage + idx + 1}
                    </td>

                    <td className="py-4 px-4">
                      <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-xl bg-emerald-50 border border-[#c8ebd2] text-[#14532d] font-black text-xs flex items-center justify-center shrink-0">
                    ...
                    <td className="py-4 px-4">
                      <span className="inline-flex items-center px-2.5 py-0.5 rounded-md text-[10.5px] font-bold bg-[#effaf2] text-[#14532d] capitalize border border-[#c8ebd2]/60">
                        {item.role || 'subadmin'}
                      </span>
                    </td>
                    ...
                    <td className="py-4 px-4 text-center">
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 text-[11px] font-bold text-[#14532d] hover:text-[#0f3e21] bg-[#effaf2] hover:bg-[#dcfce7] px-3 py-1.5 rounded-xl border border-[#c8ebd2]/80 transition-all cursor-pointer shadow-2xs"
                      >
      ...
      {permUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-fadeIn">
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-5xl h-[85vh] flex flex-col overflow-hidden">
            {/* Modal Header */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#c8ebd2]/60 bg-[#effaf2] text-[#14532d]">
              <h3 className="text-base font-extrabold flex items-center gap-2 text-[#14532d]">
            ...
            <div className="px-6 py-4 bg-slate-50 border-b border-slate-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div>
                <h4 className="text-base font-extrabold text-slate-900">{permUser.name}</h4>
              ...
              <div className="flex items-center gap-3 w-full sm:w-auto">
                <div className="flex-1 sm:flex-initial">
                  ...
                  <select
                    ...
                    className="w-full sm:w-72 px-3 py-1.5 text-xs bg-white border border-slate-300 rounded-xl focus:outline-hidden focus:ring-1 focus:ring-emerald-500 focus:border-emerald-600"
                  >
                ...
                <button
                  ...
                  className="mt-5 px-3.5 py-1.5 bg-slate-600 hover:bg-slate-700 text-white rounded-xl text-xs font-bold transition-colors shadow-2xs whitespace-nowrap cursor-pointer"
                >
                  Select None
                </button>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <table className="w-full text-left text-xs border border-[#c8ebd2]/80 rounded-xl overflow-hidden">
                <thead className="bg-[#effaf2] border-b-2 border-[#c8ebd2] text-[#14532d] font-extrabold uppercase tracking-wider sticky top-0 z-10 shadow-xs">
                  <tr>
                    <th className="py-3.5 px-4 w-[46%] bg-[#effaf2]">Module</th>
                    <th className="py-3.5 px-4 text-center bg-[#effaf2]">
                    ...
                    <th className="py-3.5 px-4 text-center bg-[#effaf2]">
                      <div className="flex flex-col items-center gap-1">
                        <span className="text-blue-700 font-extrabold">View</span>
                        <label className="relative inline-flex items-center cursor-pointer">
                          <input
                            type="checkbox"
                    ...
                    <th className="py-3.5 px-4 text-center bg-[#effaf2]">
                      <div className="flex flex-col items-center gap-1">
                        <span className="text-emerald-700 font-extrabold">Add</span>
                        <label className="relative inline-flex items-center cursor-pointer">
                          <input
                            type="checkbox"
                    ...
                    <th className="py-3.5 px-4 text-center bg-[#effaf2]">
                      <div className="flex flex-col items-center gap-1">
                        <span className="text-amber-700 font-extrabold">Edit</span>
                        <label className="relative inline-flex items-center cursor-pointer">
                          <input
                            type="checkbox"
                    ...
                    <th className="py-3.5 px-4 text-center bg-[#effaf2]">
                      <div className="flex flex-col items-center gap-1">
                        <span className="text-rose-700 font-extrabold">Delete</span>
                        <label className="relative inline-flex items-center cursor-pointer">
                          <input
                            type="checkbox"
                ...
                <tbody className="divide-y divide-slate-200">
                  {PERMISSION_MODULES.map((mod) => {
                    ...
                    return (
                      <tr key={mod.key} className="hover:bg-[#effaf2]/40 transition-colors">
            ...
            <div className="p-4 border-t border-slate-200 bg-slate-50 flex items-center justify-end gap-3">
              <button
                type="button"
                onClick={() => setPermUser(null)}
                className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-bold transition-colors shadow-2xs cursor-pointer border border-slate-300"
              >
                Close
              </button>
              <button
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xs animate-fadeIn">
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-100 w-full max-w-lg overflow-hidden">
            <div className="flex items-center justify-between p-5 border-b border-[#c8ebd2]/60 bg-[#effaf2]">
              <h3 className="text-base font-extrabold text-[#14532d]">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3.5 py-2 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:outline-hidden focus:ring-1 focus:ring-emerald-500 focus:border-emerald-600 focus:bg-white transition-all capitalize font-medium"
                  >
                    <option value="admin">Admin</option>
                    <option value="subadmin">Sub Admin</option>
                    <option value="author">Author</option>
