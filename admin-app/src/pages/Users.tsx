  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (res.ok && json.status) {
        ...
        showToast('error', json.message || 'Failed to fetch users');
      }
    } catch {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-xl border border-slate-200/80 shadow-sm">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 tracking-tight flex items-center gap-2">
            <UsersIcon className="w-6 h-6 text-indigo-600" /> Admin Users & Staff
          </h1>
          <p className="text-sm text-slate-500 mt-1">
            Manage admin users, roles, staff departments, and access credentials.
          </p>
        </div>
              ...
              {loading ? (
                ...
                paginated.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="py-3.5 px-4 font-medium text-indigo-600">{item.email}</td>
                    <td className="py-3.5 px-4">
                      <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-indigo-50 text-indigo-700 uppercase">
                        {item.role || 'subadmin'}
                      </span>
                    </td>
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              ...
              <div>
                ...
                <input
                  type="email"
                  required
                  placeholder="john@educationmalaysia.in"
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 uppercase mb-1.5">
                  Password {editingId && <span className="text-slate-400 font-normal">(Leave blank to keep current password)</span>}
                </label>
                <input
                  type="password"
                  required={!editingId}
