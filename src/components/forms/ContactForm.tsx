...
export default function ContactForm() {
  ...
  if (status === 'done') {
    return (
      <div className="bg-white rounded-[32px] p-12 border border-green-100 shadow-2xl text-center space-y-6">
        ...
        <p className="text-slate-500 font-medium max-w-sm mx-auto leading-relaxed text-lg">Thank you for reaching out. Our experts will get back to you shortly.</p>
        <button onClick={() => setStatus('idle')} className="text-blue-600 font-bold hover:underline pt-4">Send another message</button>
      </div>
    )
  }

  return (
    <div className="bg-white rounded-[32px] p-8 md:p-12 border border-gray-100 shadow-2xl shadow-blue-900/5 transition-all">
      <div className="flex items-center gap-5 mb-12">
        ...
        <div>
          ...
          <p className="text-[10px] md:text-xs text-slate-400 font-black uppercase tracking-widest mt-1">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="relative group">
          ...
          <textarea
            ...
            className="w-full pl-12 pr-6 py-5 bg-slate-50 border border-slate-50 rounded-[2rem] text-sm font-bold text-slate-900 placeholder-slate-300 focus:ring-2 focus:ring-blue-600/10 outline-none transition-all resize-none"
          />
        </div>

        <button
          type="submit" disabled={status === 'loading'}
          className="w-full bg-blue-600 text-white py-5 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-blue-700 transition-all active:scale-[0.98] shadow-2xl shadow-blue-600/20 disabled:opacity-50 flex items-center justify-center gap-3"
        >
