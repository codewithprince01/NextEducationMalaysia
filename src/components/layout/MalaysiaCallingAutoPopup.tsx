...
export default function MalaysiaCallingAutoPopup() {
  ...
  useEffect(() => {
    ...
    const dismissals = Number.parseInt(localStorage.getItem(POPUP_DISMISS_KEY) || '0', 10)
  ...
  const handleClose = () => {
    ...
    if (typeof window === 'undefined') return
    if (localStorage.getItem('scholarshipFormSubmitted') === 'true') return
    const current = Number.parseInt(localStorage.getItem(POPUP_DISMISS_KEY) || '0', 10)
    localStorage.setItem(POPUP_DISMISS_KEY, String(current + 1))
  }
