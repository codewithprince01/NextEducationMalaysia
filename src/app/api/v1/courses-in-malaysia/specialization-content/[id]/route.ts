...
export const GET = withMiddleware(checkApiKey)(async (req: NextRequest, { params }: { params: { id: string } }) => {
  try {
    ...
    const result = await malaysiaDiscoveryService.getContentDescription('specialization', params.id, { nou, noc });
    return apiSuccess(result, 'Specialization content fetched successfully');
  } catch (error: any) {
    return apiError(error.message || 'Failed to fetch specialization content', 500);
  }
});
