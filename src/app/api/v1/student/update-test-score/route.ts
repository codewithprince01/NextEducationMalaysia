...
import { testScoreSchema } from '@/backend/validators/profile';
...
export const POST = withMiddleware(checkApiKey)(async (request: Request) => {
  ...
  try {
    ...
    const validatedData = testScoreSchema.parse(body);

    const result = await studentProfileService.updateTestScore(authResult.student.sub, validatedData);
