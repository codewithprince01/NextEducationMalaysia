...
import { eduSummarySchema } from '@/backend/validators/profile';
...
export const POST = withMiddleware(checkApiKey)(async (request: NextRequest) => {
  ...
  try {
    ...
    const validatedData = eduSummarySchema.parse(body);

    const result = await studentProfileService.updateEduSum(authResult.student.sub, validatedData);
    ...
    return apiError(error.message || 'Failed to update education summary', 500);
  }
});
