...
import { satSchema } from '@/backend/validators/profile';
...
export const POST = withMiddleware(checkApiKey)(async (request: Request) => {
  ...
  try {
    ...
    const validatedData = satSchema.parse(body);

    const result = await studentProfileService.updateSAT(authResult.student.sub, validatedData);
    ...
    return apiError(error.message || 'Failed to update SAT score', 500);
  }
});
