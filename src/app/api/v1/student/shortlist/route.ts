...
export const GET = withMiddleware(checkApiKey)(async (request: Request) => {
  ...
  try {
    const result = await studentProfileService.getShortlistedPrograms(authResult.student.sub);
    return apiSuccess(result.data, result.message);
  } catch (error: any) {
    return apiError(error.message || 'Failed to fetch shortlisted programs', 500);
  }
});
