...
export async function POST(req: Request) {
  try {
    ...
    for (const row of rows) {
      const name = String(row['specialization_name'] || row['name'] || row['Specialization Name'] || '').trim();
      const categoryIdRaw = row['course_category_id'] || row['category_id'] || row['Course Category ID'] || '';
      ...
      const existing: any[] = await prisma.$queryRawUnsafe(
        `SELECT id FROM course_specializations WHERE name = ? AND website = 'MYS' LIMIT 1`,
        name
      );
      ...
      if (existing.length === 0) {
        ...
        await prisma.$executeRawUnsafe(
          `INSERT INTO course_specializations 
           (course_category_id, name, slug, courses_description, website, status, created_at, updated_at)
           VALUES (?, ?, ?, ?, 'MYS', 1, ?, ?)`,
          categoryId,
          name,
          slug,
    ...
    return NextResponse.json({ status: false, message: 'Failed to import specializations', error: error.message }, { status: 500 });
  }
}
