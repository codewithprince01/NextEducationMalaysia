  ...
  try {
    const rows: any[] = await prisma.$queryRawUnsafe(
      `SELECT s.*,
              ...
              COALESCE(sf.cnt, 0) AS faqs_count
       FROM scholarships s 
       LEFT JOIN (SELECT scholarship_id, COUNT(*) as cnt FROM scholarship_contents GROUP BY scholarship_id) sc ON sc.scholarship_id = s.id
       LEFT JOIN (SELECT scholarship_id, COUNT(*) as cnt FROM scholarship_faqs GROUP BY scholarship_id) sf ON sf.scholarship_id = s.id
       WHERE s.website = 'MYS' 
       ORDER BY s.id DESC`,
    );
...
export async function POST(req: Request) {
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      slug = body.slug || "";
      active_status =
        body.active_status !== undefined ? String(body.active_status) : "1";
      type = body.type || "";
    ...
    await recordAuditLog({
      ...
      module: 'scholarships',
      recordId: newId,
      description: `Created scholarship '${title}'`,
      newValues: { title, slug: slugVal, active_status, type, page_type },
    });
