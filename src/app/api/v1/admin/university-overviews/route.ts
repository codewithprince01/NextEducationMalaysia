...
export async function GET(request: Request) {
  try {
    ...
    const formatted = overviews.map((o) => ({
      ...o,
      university: o.university_name
        ? { id: o.university_id, name: o.university_name }
        : undefined,
    }));
    ...
    return NextResponse.json(
      {
        success: false,
        error: error.message || "Failed to fetch university overviews",
      },
      { status: 500 },
    );
  }
...
export async function POST(request: Request) {
  try {
    ...
    const [maxRes]: any[] = await prisma.$queryRawUnsafe(
      `SELECT IFNULL(MAX(id), 0) + 1 AS next_id FROM university_overviews`,
    );
    const nextId = Number(maxRes?.next_id || 1);

    await prisma.$executeRawUnsafe(
      `INSERT INTO university_overviews (id, website, university_id, title, description, thumbnail_path, thumbnail_name, position, status, created_at, updated_at)
       VALUES (?, 'MYS', ?, ?, ?, ?, ?, ?, 1, ?, ?)`,
      nextId,
