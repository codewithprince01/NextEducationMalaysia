...
export async function GET(
  ...
  try {
    ...
    const rows: any[] = await prisma.$queryRawUnsafe(
      `SELECT * FROM international_student_data WHERE id = ? LIMIT 1`,
      id
    );
...
export async function PUT(
  ...
  try {
    ...
    await prisma.$executeRawUnsafe(
      `UPDATE international_student_data 
       SET year = ?, country_id = ?, count = ?, updated_at = ?
       WHERE id = ?`,
      parseInt(year, 10),
    ...
    await recordAuditLog({
      ...
      recordId: id,
      description: `Updated international student data (Year: ${year}, Country: ${country_id}, Count: ${count || 0}) (ID: ${id})`,
      oldValues,
      newValues: body,
    });
...
export async function DELETE(
  ...
  try {
    ...
    await prisma.$executeRawUnsafe(`DELETE FROM international_student_data WHERE id = ?`, id);
    ...
    await recordAuditLog({
      ...
      module: 'international-student-data',
      recordId: id,
      description: `Deleted international student data record (ID: ${id})`,
      oldValues,
    });
