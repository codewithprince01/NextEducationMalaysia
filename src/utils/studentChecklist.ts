...
export function isStaffUploaded(doc: any): boolean {
  ...
  const src = String(doc?.upload_source || "")
    .toLowerCase()
    .trim();
  if (
    src.includes("crm") ||
  ...
  const imgPath = String(doc?.imgpath || "")
    .toLowerCase()
    .trim();
  ...
  const title = String(
    doc?.document_name || doc?.doc_name || doc?.imgname || "",
  );
 ...
 */
export function matchesDocumentRequirement(
  requirementTitle: string,
  uploadedDocName: string,
): boolean {
  const req = String(requirementTitle || "")
    .toLowerCase()
    .trim();
  const doc = String(uploadedDocName || "")
    .toLowerCase()
    .trim();
  ...
  const isReqPhoto =
    req.includes("photo") ||
    req.includes("photograph") ||
    req.includes("picture") ||
    req.includes("pic");
  const isDocPhoto =
    doc.includes("photo") ||
    doc.includes("photograph") ||
    doc.includes("picture") ||
    doc.includes("pic");
  ...
  const isReq12 =
    req.includes("12th") ||
    req.includes("grade 12") ||
    req.includes("high school") ||
    req.includes("senior secondary") ||
    req.includes("intermediate") ||
    req.includes("a-level") ||
    req.includes("a level");
  const isDoc12 =
    doc.includes("12th") ||
    doc.includes("grade 12") ||
    doc.includes("high school") ||
    doc.includes("senior secondary") ||
    doc.includes("intermediate") ||
    doc.includes("a-level") ||
    doc.includes("a level");

  const isReq10 =
    req.includes("10th") ||
    req.includes("grade 10") ||
    req.includes("secondary") ||
    req.includes("matric") ||
    req.includes("o-level") ||
    req.includes("o level");
  const isDoc10 =
    doc.includes("10th") ||
    doc.includes("grade 10") ||
    doc.includes("secondary") ||
    doc.includes("matric") ||
    doc.includes("o-level") ||
    doc.includes("o level");
  ...
  const isReqEnglish =
    req.includes("english") ||
    req.includes("ielts") ||
    req.includes("toefl") ||
    req.includes("pte") ||
    req.includes("duolingo") ||
    req.includes("moi") ||
    req.includes("language");
  const isDocEnglish =
    doc.includes("english") ||
    doc.includes("ielts") ||
    doc.includes("toefl") ||
    doc.includes("pte") ||
    doc.includes("duolingo") ||
    doc.includes("moi") ||
    doc.includes("language");
  ...
  // 5. Resume / CV
  const isReqResume =
    req.includes("resume") || req.includes("cv") || req.includes("curriculum");
  const isDocResume =
    doc.includes("resume") || doc.includes("cv") || doc.includes("curriculum");
 ...
 */
export function findMatchingUploadedDoc(
  requirementTitle: string,
  documents: any[] = [],
): any | undefined {
  ...
  return documents.find((d: any) => {
    const docName = String(
      d?.doc_name || d?.document_name || d?.title || d?.imgname || "",
    ).trim();
...
export function evaluateStudentChecklist(
  ...
  if (Array.isArray(serverRequirements) && serverRequirements.length > 0) {
    const dynamicChecklist: ChecklistItem[] = serverRequirements.map(
      (r: any) => {
        ...
        const rawStatus = String(r.doc_status || uploadedDoc?.doc_status || "")
          .trim()
          .toLowerCase();
        const isApproved =
        ...
        const isRejected =
          ...
          rawStatus === "rejected" ||
          rawStatus.includes("not app") ||
          rawStatus.includes("reject");

        let isCompleted = false;
        if (isProfile) {
          const hasFather = Boolean(
            student?.father && String(student.father).trim() !== "",
          );
          const hasMother = Boolean(
            student?.mother && String(student.mother).trim() !== "",
          );
          const hasDOB = Boolean(
            student?.dob && String(student.dob).trim() !== "",
          );
          ...
          if (isRejected) {
            ...
            isCompleted = Boolean(
              uploadedDoc || isApproved || r.doc_status === "Reviewing",
            );
          }
        }
        ...
        return {
          ...
          description:
            r.description || `Required document for application processing.`,
          category: isProfile ? "Profile Details" : "Required Documents",
          priority: (r.tag === "Required" ? "high" : "recommended") as any,
          isCompleted,
          isApproved,
          isRejected,
          docStatus: r.doc_status || uploadedDoc?.doc_status,
          ...
          completedInfo: isCompleted
            ? isProfile
              ...
              : `Uploaded: ${uploadedDoc?.document_name || uploadedDoc?.doc_name || uploadedDoc?.imgname || titleClean}`
            : undefined,
        };
      },
    );
    ...
    customHasAvatar !== undefined
      ...
      : Boolean(
          (typeof window !== "undefined" &&
            localStorage.getItem("student_profile_avatar")) ||
  ...
  const hasNationality = Boolean(
    student?.nationality && String(student.nationality).trim() !== "",
  );
  const hasGender = Boolean(
    student?.gender && String(student.gender).trim() !== "",
  );
  const isGeneralInfoComplete =
    hasFather && hasMother && hasDOB && hasNationality && hasGender;

  // 9. Permanent Address & Emergency Contact
  const hasAddress = Boolean(
    student?.home_address && String(student.home_address).trim() !== "",
  );
  const hasCity = Boolean(student?.city && String(student.city).trim() !== "");
  const hasCountry = Boolean(
    student?.country && String(student.country).trim() !== "",
  );
  const hasEmergencyContact = Boolean(
    student?.home_contact_number &&
    String(student.home_contact_number).trim() !== "",
  );
  const isAddressComplete =
    hasAddress && hasCity && hasCountry && hasEmergencyContact;

  // 10. Education History Qualifications
  const hasEducationRecords =
    Array.isArray(student?.student_educations) &&
    student.student_educations.length > 0;
  ...
  const checklist: ChecklistItem[] = [
    {
      ...
      description:
        "Scanned clear copy of your international passport bio-data and address page. Mandatory by EMGS for Malaysian student visa issuance.",
      ...
      completedInfo: passportDoc
        ? `Uploaded: ${passportDoc.doc_name || passportDoc.document_name || "Passport"}`
    ...
    },
    {
      ...
      description:
        "Official Grade 12 or High School transcript and completion certificate. Required by university admission committees to evaluate eligibility.",
      category: "Required Documents",
      priority: "high",
      ...
      targetTab: "Upload Documents",
      completedInfo: highSchoolDoc
        ? `Uploaded: ${highSchoolDoc.doc_name || highSchoolDoc.document_name || "12th Certificate"}`
        : "Document uploaded",
    },
    {
      id: "passport_photo",
      ...
      description:
        "Formal passport-size photo with white background. Needed for student identification card and EMGS visa clearance.",
      category: "Required Documents",
      priority: "high",
      ...
      targetTab: "Upload Documents",
      completedInfo: photoDoc
        ? `Uploaded: ${photoDoc.doc_name || photoDoc.document_name || "Photo"}`
    ...
    },
    {
      ...
      description:
        "Grade 10 passing certificate and marksheet used for date of birth verification and academic foundation records.",
      category: "Required Documents",
      priority: "medium",
      ...
      targetTab: "Upload Documents",
      completedInfo: secondaryDoc
        ? `Uploaded: ${secondaryDoc.doc_name || secondaryDoc.document_name || "10th Certificate"}`
        : "Document uploaded",
    },
    {
      ...
      description:
        "IELTS, TOEFL, Duolingo, PTE score or Medium of Instruction (MOI) certificate to satisfy university English language requirements.",
      category: "Required Documents",
      priority: "medium",
      ...
      targetTab: "Upload Documents",
      completedInfo: englishDoc
        ? `Uploaded: ${englishDoc.doc_name || englishDoc.document_name || "English Proof"}`
    ...
    },
    {
      ...
      description:
        "An updated resume highlighting your academic achievements, extracurriculars, and skills for scholarship consideration.",
      category: "Required Documents",
      ...
      targetTab: "Upload Documents",
      completedInfo: resumeDoc
        ? `Uploaded: ${resumeDoc.doc_name || resumeDoc.document_name || "Resume"}`
        : "Resume added",
    },
    {
      ...
      targetTab: "Upload Documents",
      completedInfo: healthDoc
        ? `Uploaded: ${healthDoc.doc_name || healthDoc.document_name || "Health Form"}`
        : "Health form uploaded",
    },
    {
      id: "personal_details",
      ...
      description:
        "Father's name, mother's name, date of birth, and nationality must be filled in your student profile.",
    ...
    },
    {
      ...
      description:
        "Full residential address, postal city, country, and emergency contact phone number are required for your file.",
      category: "Profile Details",
      priority: "medium",
    ...
    },
    {
      ...
      description:
        "Add your previous school, college, passing year, and marks achieved under the Education History section.",
...
export function getFullDocUrl(doc: any): string {
  ...
  if (isCrmDoc) {
    let crmBase = "";
    if (
      /^https?:\/\//i.test(rawUploadSource) &&
      !rawUploadSource.includes("3000") &&
      !rawUploadSource.includes("educationmalaysia.in")
    ) {
      ...
      crmBase = (
        process.env.NEXT_PUBLIC_CRM_PUBLIC_URL ||
        process.env.NEXT_PUBLIC_CRM_API_URL ||
        "https://portal.britannicaoverseas.com"
      ).replace(/\/+$/, "");
    }
  ...
  const siteUrl = (process.env.NEXT_PUBLIC_SITE_URL || "")
    ...
    .replace(/\/+$/, "");
  const baseDomain =
    runtimeOrigin || siteUrl || "https://www.educationmalaysia.in";
